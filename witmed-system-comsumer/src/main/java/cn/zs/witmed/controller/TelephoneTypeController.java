...
@RestController("telephoneTypeController")
@RequestMapping("/system/telephone/type")
public class TelephoneTypeController extends BaseController {
	@Autowired
	private TelephoneTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody TelephoneType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<TelephoneType> pageVO = new PageVO<TelephoneType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<TelephoneType> queryVO = new QueryVO<TelephoneType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody TelephoneType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody TelephoneType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			TelephoneType TelephoneType = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody TelephoneType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			TelephoneType TelephoneType = transport.getByCode(code);
			if(TelephoneType == null || TelephoneType.getId().equals(query.getId())){
