...
@RestController("maritalStatusController")
@RequestMapping("/system/marital")
public class MaritalStatusController extends BaseController {
	@Autowired
	private MaritalStatusTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody MaritalStatus query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<MaritalStatus> pageVO = new PageVO<MaritalStatus>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<MaritalStatus> queryVO = new QueryVO<MaritalStatus>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody MaritalStatus query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody MaritalStatus query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			MaritalStatus MaritalStatus = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody MaritalStatus query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			MaritalStatus MaritalStatus = transport.getByCode(code);
			if(MaritalStatus == null || MaritalStatus.getId().equals(query.getId())){
