...
@RestController("drugTypeController")
@RequestMapping("/system/drug/type")
public class DrugTypeController extends BaseController {
	@Autowired
	private DrugTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody DrugType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<DrugType> pageVO = new PageVO<DrugType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<DrugType> queryVO = new QueryVO<DrugType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody DrugType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody DrugType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugType DrugType = transport.getByCode(code);
			if(DrugType ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DrugType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugType DrugType = transport.getByCode(code);
			if(DrugType == null || DrugType.getId().equals(query.getId())){
