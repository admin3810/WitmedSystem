...
@RestController("professionTypeController")
@RequestMapping("/system/profession/type")
public class ProfessionTypeController extends BaseController {
	@Autowired
	private ProfessionTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody ProfessionType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<ProfessionType> pageVO = new PageVO<ProfessionType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<ProfessionType> queryVO = new QueryVO<ProfessionType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody ProfessionType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody ProfessionType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			ProfessionType ProfessionType = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody ProfessionType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			ProfessionType ProfessionType = transport.getByCode(code);
			if(ProfessionType == null || ProfessionType.getId().equals(query.getId())){
