...
@RestController("allergicSymptomsController")
@RequestMapping("/system/allergic/symptoms")
public class AllergicSymptomsController extends BaseController {
	@Autowired
	private AllergicSymptomsTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody AllergicSymptoms query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<AllergicSymptoms> pageVO = new PageVO<AllergicSymptoms>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<AllergicSymptoms> queryVO = new QueryVO<AllergicSymptoms>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody AllergicSymptoms query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody AllergicSymptoms query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			AllergicSymptoms AllergicSymptoms = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AllergicSymptoms query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			AllergicSymptoms AllergicSymptoms = transport.getByCode(code);
			if(AllergicSymptoms == null || AllergicSymptoms.getId().equals(query.getId())){
