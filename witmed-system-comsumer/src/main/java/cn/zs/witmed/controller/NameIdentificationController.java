...
@RestController("nameIdentificationController")
@RequestMapping("/system/name/identification")
public class NameIdentificationController extends BaseController {
	@Autowired
	private NameIdentificationTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody NameIdentification query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<NameIdentification> pageVO = new PageVO<NameIdentification>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<NameIdentification> queryVO = new QueryVO<NameIdentification>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody NameIdentification query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody NameIdentification query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			NameIdentification NameIdentification = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody NameIdentification query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			NameIdentification NameIdentification = transport.getByCode(code);
			if(NameIdentification == null || NameIdentification.getId().equals(query.getId())){
