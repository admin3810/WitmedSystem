...
@RestController("drugUseController")
@RequestMapping("/system/drug/type")
public class DrugUseController extends BaseController {
	@Autowired
	private DrugUseTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody DrugUse query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<DrugUse> pageVO = new PageVO<DrugUse>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<DrugUse> queryVO = new QueryVO<DrugUse>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody DrugUse query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody DrugUse query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugUse DrugUse = transport.getByCode(code);
			if(DrugUse ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DrugUse query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugUse DrugUse = transport.getByCode(code);
			if(DrugUse == null || DrugUse.getId().equals(query.getId())){
