...
public class IdTypeController extends BaseController {
	...
	@PostMapping("save")
	public ResponseVO save(@RequestBody IdType query) throws Exception {
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody IdType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			IdType idType = transport.getByCode(code);
			if(idType == null || idType.getId().equals(query.getId())){
