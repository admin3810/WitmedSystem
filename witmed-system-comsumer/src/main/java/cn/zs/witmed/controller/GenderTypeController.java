...
@RestController("genderTypeController")
@RequestMapping("/system/gender/type")
public class GenderTypeController extends BaseController {
	@Autowired
	private GenderTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody GenderType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<GenderType> pageVO = new PageVO<GenderType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<GenderType> queryVO = new QueryVO<GenderType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody GenderType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody GenderType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			GenderType GenderType = transport.getByCode(code);
			if(GenderType ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody GenderType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			GenderType GenderType = transport.getByCode(code);
			if(GenderType == null || GenderType.getId().equals(query.getId())){
