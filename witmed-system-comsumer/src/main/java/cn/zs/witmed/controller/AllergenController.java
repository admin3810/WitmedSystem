...
@RestController("allergenController")
@RequestMapping("/system/drug/type")
public class AllergenController extends BaseController {
	@Autowired
	private AllergenTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody Allergen query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<Allergen> pageVO = new PageVO<Allergen>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<Allergen> queryVO = new QueryVO<Allergen>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody Allergen query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody Allergen query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			Allergen Allergen = transport.getByCode(code);
			if(Allergen ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Allergen query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			Allergen Allergen = transport.getByCode(code);
			if(Allergen == null || Allergen.getId().equals(query.getId())){
