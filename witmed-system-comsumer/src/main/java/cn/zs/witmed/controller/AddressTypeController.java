...
@RestController("addressTypeController")
@RequestMapping("/system/address")
public class AddressTypeController extends BaseController {
	@Autowired
	private AddressTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody AddressType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<AddressType> pageVO = new PageVO<AddressType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<AddressType> queryVO = new QueryVO<AddressType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody AddressType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody AddressType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			AddressType AddressType = transport.getByCode(code);
			if(AddressType ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AddressType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			AddressType AddressType = transport.getByCode(code);
			if(AddressType == null || AddressType.getId().equals(query.getId())){
