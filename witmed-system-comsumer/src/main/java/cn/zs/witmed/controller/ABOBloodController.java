...
@RestController("aBOBloodController")
@RequestMapping("/system/abo")
public class ABOBloodController extends BaseController {
	@Autowired
	private ABOBloodTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody ABOBlood query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<ABOBlood> pageVO = new PageVO<ABOBlood>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<ABOBlood> queryVO = new QueryVO<ABOBlood>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody ABOBlood query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody ABOBlood query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			ABOBlood ABOBlood = transport.getByCode(code);
			if(ABOBlood ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody ABOBlood query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			ABOBlood ABOBlood = transport.getByCode(code);
			if(ABOBlood == null || ABOBlood.getId().equals(query.getId())){
