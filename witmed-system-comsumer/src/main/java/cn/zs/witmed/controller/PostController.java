...
public class PostController extends BaseController {
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody Post query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<Post> pageVO = new PageVO<Post>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<Post> queryVO = new QueryVO<Post>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody Post query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody Post query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			Post Post = transport.getByCode(code);
			if(Post ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Post query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			Post Post = transport.getByCode(code);
			if(Post == null || Post.getId().equals(query.getId())){
