...
public class NationController extends BaseController {
	@Autowired
	private NationTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
		...
		// 创建 QueryVO 对象
		QueryVO<Nation> queryVO = new QueryVO<Nation>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody Nation query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody Nation query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			Nation Nation = transport.getByCode(code);
			if(Nation ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Nation query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			Nation Nation = transport.getByCode(code);
			if(Nation == null || Nation.getId().equals(query.getId())){
