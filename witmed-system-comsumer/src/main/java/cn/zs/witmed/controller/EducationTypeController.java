...
@RestController("educationTypeController")
@RequestMapping("/system/education/type")
public class EducationTypeController extends BaseController {
	@Autowired
	private EducationTypeTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody EducationType query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<EducationType> pageVO = new PageVO<EducationType>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<EducationType> queryVO = new QueryVO<EducationType>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody EducationType query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody EducationType query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			EducationType EducationType = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody EducationType query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			EducationType EducationType = transport.getByCode(code);
			if(EducationType == null || EducationType.getId().equals(query.getId())){
