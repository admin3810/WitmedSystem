...
@RestController("allergiesController")
@RequestMapping("/system/drug/type")
public class AllergiesController extends BaseController {
	@Autowired
	private AllergiesTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody Allergies query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<Allergies> pageVO = new PageVO<Allergies>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<Allergies> queryVO = new QueryVO<Allergies>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody Allergies query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody Allergies query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			Allergies Allergies = transport.getByCode(code);
			if(Allergies ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Allergies query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			Allergies Allergies = transport.getByCode(code);
			if(Allergies == null || Allergies.getId().equals(query.getId())){
