...
@RestController("individualRiskController")
@RequestMapping("/system/individual")
public class IndividualRiskController extends BaseController {
	@Autowired
	private IndividualRiskTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody IndividualRisk query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<IndividualRisk> pageVO = new PageVO<IndividualRisk>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<IndividualRisk> queryVO = new QueryVO<IndividualRisk>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody IndividualRisk query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody IndividualRisk query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			IndividualRisk IndividualRisk = transport.getByCode(code);
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody IndividualRisk query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			IndividualRisk IndividualRisk = transport.getByCode(code);
			if(IndividualRisk == null || IndividualRisk.getId().equals(query.getId())){
