...
@RestController("drugUseRateController")
@RequestMapping("/system/drug/type")
public class DrugUseRateController extends BaseController {
	@Autowired
	private DrugUseRateTransport transport;
	@PostMapping("/page/{pageNum}/{pageSize}")
	public ResponseVO getPageByQuery(@PathVariable("pageNum")Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                                 @RequestBody DrugUseRate query) throws Exception {
		// 创建 PageVO 对象
		System.out.println(123);
		System.out.println(query);
		PageVO<DrugUseRate> pageVO = new PageVO<DrugUseRate>(pageNum, pageSize);
		// 创建 QueryVO 对象
		QueryVO<DrugUseRate> queryVO = new QueryVO<DrugUseRate>();
	...
	@PostMapping("/query")
	public ResponseVO getByQuery(@RequestBody DrugUseRate query) throws Exception {
		return ResponseVO.getSuccessResponseVO(transport.getListByQuery(query));
	}


	@PostMapping("save")
	public ResponseVO save(@RequestBody DrugUseRate query) throws Exception {
		String code =query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugUseRate DrugUseRate = transport.getByCode(code);
			if(DrugUseRate ==null){
	...
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DrugUseRate query) throws Exception {
		String code = query.getCode();
		if (code !=null&&!"".equals(code)){
			DrugUseRate DrugUseRate = transport.getByCode(code);
			if(DrugUseRate == null || DrugUseRate.getId().equals(query.getId())){
